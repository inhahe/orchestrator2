...
| `/btw <question>` | Side question answered **while the session keeps working**. Forks the conversation (same context, new session id) into a second CLI, so the live turn is untouched and never sees it. Renders inline as a distinct side exchange. The fork cannot run Bash, Write or Edit, because it runs concurrently with a turn that may be mid-edit on the same files (the block is by tool name, so a write-capable MCP server from your settings would still be reachable). Its throwaway transcript is deleted once it answers. One at a time |
